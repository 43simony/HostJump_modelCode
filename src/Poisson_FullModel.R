...
p3 <- ggplot() +
  geom_line(data = dat2[dat2$lambda==1,], aes(x = phi, y = prob), color = '#4490FEFF') +
  geom_line(data = dat2[dat2$lambda==4,], aes(x = phi, y = prob), color = '#EA4F0DFF') +
  facet_wrap( ~ T_lim, nrow = 1 ) +
...
x=as.data.frame(matrix(nrow=0,ncol=3)); names(x)<-c('lambda','t_spill', 'T_P')

T_F=10; lambda_lo = 0.8; lambda_hi = 2
TP_vec = c(2,10)
for(T_P in TP_vec){
  
  ## simulate spillover events over 0:T_P+T_F
  M=round(lambda_lo*(T_P+T_F))
  ## populate data frame
  if(M>0){
    t_vals = seq(-1*T_P,T_F,length=M+2)[-1] 
    t_vals = t_vals[-length(t_vals)]
    x_F = expand.grid(lambda = lambda_lo, t_spill = t_vals) ## simulate spillover times 
    x_F$T_P = T_P
  }else{
    x_F = data.frame(lambda = lambda_lo, t_spill = (T_F-T_P)*0.5, T_P = T_P) ## Ensure at least one theoretical future spillover
  }
  x<-rbind(x,x_F) ## merge into one object
  
  
  ## simulate spillover events over 0:T_P+T_F
  M=round(lambda_hi*(T_P+T_F))
  ## populate data frame
  if(M>0){
    t_vals = seq(-1*T_P,T_F,length=M+2)[-1] 
    t_vals = t_vals[-length(t_vals)]
    x_F = expand.grid(lambda = lambda_hi, t_spill = t_vals) ## simulate spillover times 
    x_F$T_P = T_P
  }else{
    x_F = data.frame(lambda = lambda_hi, t_spill = (T_F-T_P)*0.5, T_P = T_P) ## Ensure at least one theoretical future spillover
  }
  x<-rbind(x,x_F) ## merge into one object
...
x$lambda[x$lambda==lambda_lo] = 1 ## setting lower dummy y value to condense plot size
x$lambda[x$lambda==lambda_hi] = 1.15 ## setting upper dummy y value to condense plot size

df <- expand.grid(T_P = c(0,1,2,5), lambda = c(1,1.15))
df$negLim = -1*df$T_P
figE <- ggplot() +
  
  geom_rect(data = x, aes(xmin = -Inf, xmax = 0, ymin = -Inf, ymax = Inf), fill = "gray55", alpha = 0.5) +
  geom_rect(data = x, aes(xmin = 0, xmax = Inf, ymin = -Inf, ymax = Inf), fill = "gray90", alpha = 0.5) +
  geom_point(data = x, aes(x=t_spill, y=lambda, color = lambda_val), size = 4, shape = 1, stroke = 1.2)  +
  scale_color_manual(values=c('#4490FEFF', '#EA4F0DFF'), breaks = c("0.8","2")) +
  annotate("segment",x=0,xend=0, y=0.9,yend=1.3, size=1, linetype = 'dashed', alpha = 0.95, color = "gray0") +
  
  geom_point(data = df, aes(x=negLim, y=lambda, color = lambda_val), color = "#000000", size = 13, shape = "*") +
  geom_segment(data = x, aes(x = -1 * T_P, xend = T_F, y = 1, yend = 1), size = 1) +
  geom_segment(data = x, aes(x = -1 * T_P, xend = T_F, y = 1.15, yend = 1.15), size = 1) +
  
  scale_x_continuous(limits = c(-1*max(TP_vec), T_F)) +
  scale_y_continuous(limits = c(0.9,1.3)) +
  facet_wrap( ~ T_P, nrow = 1 ) +
  annotate("text", x = -2.5, y = 1.25, label = "past", size = 6, hjust = 0.5, parse = T) +
  annotate("text", x = 2.5, y = 1.25, label = "future", size = 6, hjust = 0.5, parse = T) +
...
ggsave(filename = paste("./figures/conFig_E.png"), plot = figE, width = 10.00, height = 3.5, units = "in")
...
vals = expand.grid(seq(0, max, by = step), seq(0, max, by = step))
names(vals) = c('lambda', 'lambda_f') ## names must be adjusted to match parameter name in "pars" data frame
...
mixtureDistn <- data.frame( a = c(0.1, 1),
                            b =  c(10, 1),
                            weights = c(0.8, 0.2) )
tmp = data.frame(phi = phi_vec, density = betaMix(x = phi_vec, weights = mixtureDistn$weights, shape1 = mixtureDistn$a, shape2 = mixtureDistn$b )$density, 
...

{
  ...
  int_map1 <- ggplot() +
    ...
    xlab(" ") +
    ylab (expression( "expected number of\n"*"future spillovers"*(lambda[F]*t[F]) ) ) +
...
L_curves <- ggplot() +
  ...
  annotate("segment", x=-Inf, xend=Inf, y=-Inf, yend=-Inf, linewidth = 1) +
  annotate("segment", x=-Inf, xend=-Inf, y=-Inf, yend=Inf, linewidth = 1) +
  labs(x = expression( "Past history "*(t[P])), 
